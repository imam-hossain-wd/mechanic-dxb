...
export function ClientReviewsSection() {
  ...
  // Hydration mismatch এড়াতে useEffect-এর মাধ্যমে deterministic data তৈরি
  useEffect(() => {
    const updatedReviews = reviews.map((review, index) => ({
      ...
      helpful: 15 + index * 3, // Math.random() এর বদলে ফিক্সড লজিক যা সারভার/ক্লায়েন্ট ঝামেলা করবে না
      avatarColor: AVATAR_COLORS[index % 6],
      isFeatured: index === 0 || index === 3,
    }));
    setFormattedReviews(updatedReviews);
  }, []);
  ...
  const displayReviews = formattedReviews.length > 0 ? formattedReviews : reviews;
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between pt-3 border-t border-border/50">
                      <div className="flex items-center gap-2.5">
                        <div className={cn(
                          "flex h-9 w-9 items-center justify-center rounded-full bg-gradient-to-br text-sm font-bold text-white",
                          review.avatarColor || AVATAR_COLORS[index % 6]
                        )}>
                          {review.avatar || review.name?.charAt(0)}
                        </div>
                        <div>
